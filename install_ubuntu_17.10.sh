...
STRING1="Make sure you double check before hitting enter! Only one shot at this, no pressure."
STRING2="Did you find this helpful? "
STRING3="Great, maybe go do something awesome today. Donate to charity or something."
...
masternode=1
' | sudo -E tee ~/.seci/seci.conf >/dev/null 2>&1
sudo chmod 0600 ~/.seci/seci.conf

#Starting coin
(
  crontab -l 2>/dev/null
  echo '@reboot sleep 30 && secid -daemon -shrinkdebugfile'
) | crontab
(
  crontab -l 2>/dev/null
  echo '@reboot sleep 60 && seci-cli startmasternode local false'
) | crontab
northernd -daemon
...
seci-cli startmasternode local false
seci-cli masternode status
